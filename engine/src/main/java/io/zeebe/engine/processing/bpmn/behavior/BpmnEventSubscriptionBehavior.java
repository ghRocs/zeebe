...
public final class BpmnEventSubscriptionBehavior {
  ...
  public void triggerStartEvent(final BpmnElementContext context) {
    final long processKey = context.getProcessKey();
    final long processInstanceKey = context.getProcessInstanceKey();

    final var process = processState.getProcessByKey(context.getProcessKey());
    if (process == null) {
      // this should never happen because processs are never deleted.
      throw new BpmnProcessingException(
          context, String.format(NO_PROCESS_FOUND_MESSAGE, processKey));
    }

    final var triggeredEvent = eventScopeInstanceState.peekEventTrigger(processKey);
    if (triggeredEvent == null) {
      throw new BpmnProcessingException(
          context, String.format(NO_TRIGGERED_EVENT_MESSAGE, processKey));
    }
    ...
    eventScopeInstanceState.deleteTrigger(processKey, triggeredEvent.getEventKey());
  }
