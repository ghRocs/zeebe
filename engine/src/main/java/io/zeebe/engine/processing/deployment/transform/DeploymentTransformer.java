    ...
    for (final Process process : processes) {
      if (process.isExecutable()) {
        ...
        if (isDuplicateOfLatest(deploymentResource, resourceDigest, lastProcess, lastDigest)) {
          processKey = lastProcess.getKey();
          version = lastProcess.getVersion();
        } else {
          processKey = keyGenerator.nextKey();
        ...
        final var processRecord = deploymentEvent.processs().add();
        processRecord
        ...
        stateWriter.appendFollowUpEvent(processKey, ProcessIntent.CREATED, processRecord);
      }
    }
  }
