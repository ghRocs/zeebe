...
public final class LargeMessageSizeTest {
  ...
  public void shouldDeployLargeProcess() {
    ...
    final var processKey = deployment.getProcesss().get(0).getProcessKey();

    // then
    final var processInstanceEvent =
        CLIENT_RULE.getClient().newCreateInstanceCommand().processKey(processKey).send().join();
  ...
  public void shouldCreateInstanceWithLargeVariables() {
    // given
    final var processKey = CLIENT_RULE.deployProcess(process(jobType));
