...
public class SmallMessageSizeTest {
  ...
  public void shouldSkipJobsThatExceedMessageSize() {
    // given (two processs, the first has variables too big to fit into a message, the second fits
    // into a message)
    final var processKey = CLIENT_RULE.deployProcess(process(JOB_TYPE));

    // process with variables that are greater than the message size
    final var processInstanceKey1 = CLIENT_RULE.createProcessInstance(processKey);
    ...
    final var processInstanceKey2 = CLIENT_RULE.createProcessInstance(processKey);
  ...
  public void shouldActivateJobIfRequestVariablesFitIntoMessageSize() {
    // given (processs with variables too big to fit into a message)
    final var processKey = CLIENT_RULE.deployProcess(process(JOB_TYPE));
